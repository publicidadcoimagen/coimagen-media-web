...
// isEs here is the diagnosis's own language (data.language), not the
// site-wide toggle — see the pageIsEs comment in DiagnosticoResultado.
function ReadyDiagnosis({
  data,
  analysis,
  tier,
  sortedTasks,
  ...
  data: DigitalDiagnosisResult;
  analysis: NonNullable<DigitalDiagnosisResult["result"]>;
  tier: { name: string; color: string; border: string };
  sortedTasks: NonNullable<DigitalDiagnosisResult["result"]>["prioritizedTasks"];
  isEs: boolean;
}) {
  return (
    <>
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-[var(--c-muted)]/70 text-xs mt-3">
          {isEs ? "Este diagnóstico fue generado en español." : "This diagnostic was generated in English."}
        </p>
      </div>
...
export default function DiagnosticoResultado() {
  ...
  // This page's chrome must follow the language the diagnosis was actually
  // generated in (data.language), not the site-wide ES/EN toggle — the
  // AI-generated content (summary, issues, prioritizedTasks) can't be
  // regenerated on the fly, so following the toggle would produce a mixed
  // page (translated chrome + untranslated AI content). Falls back to the
  // global toggle only before the diagnosis has loaded (loading/error/
  // not-found states have no diagnosis language to anchor to).
  const pageIsEs = data ? data.language === "es" : isEs;

  useEffect(() => {
    const title = pageIsEs ? "Tu Diagnóstico Digital — Coimagen Media Agency" : "Your Digital Diagnostic — Coimagen Media Agency";
    document.title = title;
    document.querySelector('meta[name="description"]')?.setAttribute(
      "content",
      pageIsEs ? "Resultados de tu diagnóstico digital gratuito." : "Your free digital diagnostic results."
    );
  }, [pageIsEs]);

  const analysis = data?.result ?? null;
  const tier = analysis ? getScoreTier(analysis.overallScore, pageIsEs) : null;
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        {state === "ready" && data && (!analysis || !tier) && (
          <div className="glass border border-white/[0.08] rounded-2xl p-10 text-center">
            <div className="text-4xl mb-4">⏳</div>
            <h1 className="text-white font-black text-xl mb-2">
              {pageIsEs ? "Tu diagnóstico se está generando" : "Your diagnostic is still generating"}
            </h1>
            <p className="text-[var(--c-muted)] text-sm">
              {pageIsEs ? "Vuelve a revisar en unos minutos." : "Check back again in a few minutes."}
            </p>
          </div>
        )}

        {state === "ready" && data && analysis && tier && (
          <ReadyDiagnosis data={data} analysis={analysis} tier={tier} sortedTasks={sortedTasks} isEs={pageIsEs} />
        )}
